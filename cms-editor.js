...
(function () {
  ...
  function renderImageEditor(el, container) {
    ...
    container.innerHTML = `
      <div class="cms-img-preview-box">
        <img id="cms-img-preview" src="${currentSrc}" alt="Previsualización"/>
      </div>
      <div class="cms-form-group">
        <label class="cms-form-label">Elegir de fotos existentes:</label>
        <select id="cms-select-preset" class="cms-select">
          <option value="">-- Seleccionar imagen del sitio --</option>
          ${presetImages.map(img => `<option value="${img}" ${currentSrc === img ? 'selected' : ''}>${img}</option>`).join('')}
        </select>
      </div>
      <div class="cms-form-group">
        <label class="cms-form-label">O ruta / URL de la imagen:</label>
        <input type="text" id="cms-img-src" class="cms-input" value="${currentSrc}" placeholder="Ej: 1Carrusel.jpg o URL web"/>
      </div>
      <div class="cms-form-group">
        <label class="cms-form-label">Subir nueva foto desde tu equipo:</label>
        <input type="file" id="cms-file-upload" accept="image/*" class="cms-input"/>
        <div class="cms-form-help">Si seleccionas una imagen de tu computadora, se adaptará inmediatamente para la web.</div>
      </div>
      ${isImg ? `
      <div class="cms-form-group">
        <label class="cms-form-label">Descripción (Alt SEO):</label>
  ...
  function renderLinkEditor(el, container) {
    ...
    // Comprobar si es un enlace de WhatsApp
    ...
    document.getElementById('cms-modal-save-btn').onclick = function () {
      ...
      if (el.querySelector('span')) {
        ...
        // Preservar SVGs dentro del botón si existen
  ...
  function renderCardEditor(el, container) {
    ...
    container.innerHTML = `
      ...
      <div class="cms-form-group">
        <label class="cms-form-label">Foto del producto (Nombre o URL):</label>
        <input type="text" id="cms-prod-img" class="cms-input" value="${escapeHtml(imgSrc)}"/>
      </div>
      <div class="cms-form-group">
    ...
    document.getElementById('cms-modal-save-btn').onclick = function () {
      ...
      const newImg = document.getElementById('cms-prod-img').value.trim();
